...
const code = fs.readFileSync("./multiply.gunwoo").toString();
...
function runGunwoo_lang(code) {
    ...
    for (let i = 0; i < code.length; i++) {
        ...
        if (conditi) {
            ...
            // 변수
            variables[variable[1].length] = gunnumToNumber(
                line.replace(/거(어+)언/, ""),
            );
        } else if (numlog) {
            // 숫자 콘솔 출력
            ...
            console.log(
                String.fromCharCode(
                    gunnumToNumber(line.split("병")[1].split("신")[0]),
                ),
            );
        } else if (readline) {
            // 사용자 입력
        ...
        string.split("").forEach((e) => {
            switch (e) {
                case "ㅇ":
                    ...
                    break;
                case "우":
                    if (variables[variableCount] == undefined)
                        throw new Error(
                            "변수가 존재하지 않습니다: " +
                                "ㅇ".repeat(variableCount) +
                                "우",
                        );
                    ismultiply = false;
                    if (sign == "") {
